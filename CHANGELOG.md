...
- `transferEntitlements` gains the `sourceIsNewer` guard that `aliasEntitlements` already had. Out-of-order `TRANSFER` events can no longer regress a freshly-renewed `expiresAtMs` on the destination when a source with an older expiry is moved in.
