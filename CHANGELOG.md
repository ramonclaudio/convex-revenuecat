...
- **`CANCELLATION` with `cancel_reason: "SUBSCRIPTION_PAUSED"` forced `autoRenewStatus: false`.** Paused users (who intend to resume) were shown as cancelled. Now treats SUBSCRIPTION_PAUSED like refunds — leaves autoRenewStatus alone. `UNSUBSCRIBE` cancellations also now set `subscription.unsubscribeDetectedAt` so consumers can distinguish "will not renew" from refunds and billing errors.
